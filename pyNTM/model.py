...
#        pdb.set_trace()
    ...
    def update_simulation(self):
        ...
        # associated with the non-failed interfaces
        for interface_object in self.interface_objects:
            if interface_object.failed is False:
                non_failed_interfaces.add(interface_object)
                available_nodes.add(interface_object.node_object)
                available_nodes.add(interface_object.remote_node_object)
        ...
        # corresponding nodes
        non_failed_interfaces_model = Model(non_failed_interfaces, available_nodes,
                                            self.demand_objects, self.rsvp_lsp_objects)

        # Route the RSVP LSPs
        self = self._route_lsps(non_failed_interfaces_model)

        # Route the demands - change the name of the call if you use this
        self = self._route_non_lsp_demands(self.demand_objects, non_failed_interfaces_model)
    ...
    def _unique_interface_per_node(self):
        ...
        if len(exception_interfaces) > 0:
            message = "Interface names must be unique per node. \
                       The following nodes have duplicate interface names %s" % exception_interfaces
            return message
        else:
            return True
    ...
    def _make_circuits(self, return_exception=True, include_failed_circuits=True):
        ...
        paired_interfaces = ((local_node_name, remote_node_name, data)
    ...
    def get_interface_object_from_nodes(self, local_node_name, remote_node_name):
        ...
        """
        for interface in (interface for interface in self.interface_objects):
            if interface.node_object.name == local_node_name and interface.remote_node_object.name == remote_node_name:
                return interface
                break
    ...
    def add_node(self, node_object):
        ...
        if node_object.name in [node.name for node in self.node_objects]:
            message = "A node with name %s already exists in the model" % node_object.name
...
#        pdb.set_trace()
    ...
    def get_demand_object(self, source_node_name, dest_node_name, demand_name='none'):
        ...
        for demand in model_demand_iterator:
            if (demand.source_node_object.name == source_node_name and
                    demand.dest_node_object.name == dest_node_name and demand.name == demand_name):
    ...
    def get_rsvp_lsp(self, source_node_name, dest_node_name, lsp_name='none'):
        ...
        if needed_key not in (lsp._key for lsp in self.rsvp_lsp_objects):
            msg = "LSP with source node %s, dest node %s, and name %s does not exist in model" % (
                source_node_name, dest_node_name, lsp_name)
    ...
    def get_interface_object(self, interface_name, node_name):
        ...
        interface_name_list = [interface.name for interface in node_object.interfaces(self)]
        ...
        if interface_name in interface_name_list:
            ...
            msg = "Interface(%s, %s, NA) does not exist" % (interface_name, node_name)
            raise ModelException(msg)
    ...
    def get_non_failed_interface_objects(self):
        ...
        for interface in (interface for interface in self.interface_objects):
            if interface.failed is False:
                int_object = self.get_interface_object(interface.name, interface.node_object.name)
    ...
    def _get_initial_candidate_paths(self, source_node_object, dest_node_object):
        ...
        initial_candidate_paths = [interface for interface in source_node_object.interfaces(
            self) if interface.failed is False]
    ...
    def _examine_candidate_paths(self, source, dest, candidate_paths, feasible_paths):
        """
        ...
        for path in candidate_paths:
            ...
            most_recent_node_interfaces = [interface for interface in Node(
                most_recent_node).interfaces(self) if interface.failed is False]
    ...
    def get_feasible_paths_old(self, source_node_object, dest_node_object):
        ...
        data = self._get_initial_candidate_paths(source_node_object, dest_node_object)

        initial_candidate_paths = data['initial_candidate_paths']
        feasible_paths = data['feasible_paths']

        feasible_paths = self._examine_candidate_paths(
            source_node_object, dest_node_object, initial_candidate_paths, feasible_paths)
    ...
    def get_feasible_paths(self, source_node_name, dest_node_name):
        ...
        # Get the paths
        all_feasible_digraph_paths = nx.all_simple_paths(G, source_node_name, dest_node_name,)
    ...
    def _convert_nx_path_to_model_path(self, nx_graph_path):
        """
        Given a path from an networkx DiGraph, converts thatpath to a
        Model style path and returns that Model style path
        """
    ...
    def fail_node(self, node_name):
        ...
        ints_to_fail_iterator = (interface for interface in self.get_node_interfaces(node_name))
    ...
    def unfail_node(self, node_name):
        ...
        for interface in ints_to_unfail_iterator:
            ...
            if interface.remote_node_object.failed is False:
                ...
                self.unfail_interface(remote_int.name, remote_int.node_object.name, False)
    ...
    def _make_weighted_network_graph_lsps(self):
        ...
        edge_names = ((rsvp_lsp.node_object.name, rsvp_lsp.remote_node_object.name, rsvp_lsp.cost)
                      for rsvp_lsp in self.rsvp_lsp_objects)
    ...
    def create_demand_objects_from_list(self, list_of_demand_properties):
        ...
        for demand in list_of_demand_properties:
            if 'name' in demand.keys():
                ...
                demand_name = 'none'
            self.add_demand(demand['source'], demand['dest'], demand['traffic'], demand_name)
    ...
    def load_model_file(data_file):
        ...
        for interface_line in interface_lines:
            ...
            interface_set.add(Interface(name, int(cost), int(capacity), Node(node_name), Node(remote_node_name)))
