...
class WeatherMap(object):  # pragma: no cover
    ...
    def __init__(self, model):
        ...
        self.styles = {
            "all-content": {"width": "auto", "height": "auto"},
            "right_menu": {
                "width": "25vw",
                "min-width": "400px",
                "max-width": "800px",
    ...
    # #### Utility Functions #### #
    def make_json_node(
        self, x, y, id, label, midpoint=False, neighbors=[], failed=False
    ):
        """
    ...
    def format_objects_for_display(self, object_list):
        ...
        for object in object_list:
            ...
            objects.append(
                {"label": object.__repr__(), "value": json.dumps(object_info)}
            )
        return objects
    ...
    def get_sources(self, destination, object_type):
        ...
        if object_type == "demand":
            source_dest_pairs = [
                pair.split("-") for pair in self.model.parallel_demand_groups().keys()
            ]
        elif object_type == "lsp":
            source_dest_pairs = [
                pair.split("-") for pair in self.model.parallel_lsp_groups().keys()
            ]
    ...
    def create_elements(self, group_midpoints=True):
        ...
        for ckt in self.model.circuit_objects:
            ...
            node_a_y = (
                -node_a.lat * self.spacing_factor
            )  # Added - sign here, for some reason y axis is inverted
            node_a_x = node_a.lon * self.spacing_factor
            node_b_y = (
                -node_b.lat * self.spacing_factor
            )  # Added - sign here, for some reason y axis is inverted
            ...
            new_node = self.make_json_node(
                midpoint_x,
                midpoint_y,
                midpoint_label,
                midpoint_label,
                midpoint=True,
                neighbors=[node_a.name, node_b.name],
            )
            ...
            nodes.append(
                self.make_json_node(
                    node_a_x, node_a_y, node_a.name, node_a.name, failed=node_a.failed
                )
            )
            nodes.append(
                self.make_json_node(
                    node_b_x, node_b_y, node_b.name, node_b.name, failed=node_b.failed
                )
            )
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.P(
                            id="selected-lsp-output", style=self.styles["json-output"]
                        ),
                        dcc.Tabs(
                            id="tabs",
                            vertical=True,
                            ...
                            children=[
                                dcc.Tab(
                                    label="Utilization Visualization",
                                    style=self.styles["tab"],
                                    children=[
                                        dcc.Dropdown(
                                            style=self.styles["tab-content"],
                                            id="utilization-dropdown-callback",
                                            options=self.utilization_display_info(),
                                            value=[
                                                entry["value"]
                                                for entry in self.utilization_display_info()
                                            ],
                                            multi=True,
                                        )
                                    ],
                                    ...
                                    children=[
                                        html.Div(
                                            ...
                                            children=[
                                                ...
                                                dcc.Dropdown(
                                                    id="demand-source-callback",
                                                    placeholder="Select a source node",
                                                ),
                                                dcc.Dropdown(
                                                    id="demand-destination-callback",
                                                    placeholder="Select a dest node",
                                                ),
                                                dcc.RadioItems(
                                ...
                                dcc.Tab(
                                    label="Find Interfaces on Node",
                                    style=self.styles["interface-tab"],
                                    children=[
                                        html.Div(
                                            style=self.styles["tab-content"],
                                            children=[
                                                dcc.Dropdown(
                                                    id="find-node",
                                                    placeholder="Select a node by name",
                                    ...
                                    children=[
                                        html.Div(
                                            ...
                                            children=[
                                                ...
                                                dcc.Dropdown(
                                                    id="lsp-source-callback",
                                                    placeholder="Select a source node",
                                                ),
                                                dcc.Dropdown(
                                                    id="lsp-destination-callback",
                                                    placeholder="Select a dest node",
                                                ),
                                                dcc.RadioItems(
    ...
    def create_weathermap(self):  # noqa C901
        ...
        def interfaces_on_node(node):
            ...
            if node:
                ...
                interface_info_to_display = format_interfaces_for_display(
                    interfaces_on_node
                )
                ...
                if (
                    ...
                    interface_info_to_display.insert(
                        0, null_choice
                    )  # Should give user option to clear selected item
        ...
        @app.callback(
            Output("cytoscape-prototypes", "stylesheet"),
            [
                Input(
                    component_id="cytoscape-prototypes",
                    component_property="selectedEdgeData",
                ),
        ...
        def update_stylesheet(
            ...
            if (
                ...
                try:
                    dmds = self.model.parallel_demand_groups()[
                ...
                for interface in interfaces_to_highlight:
                    ...
                    new_entry = {
                        "selector": 'edge[circuit_id="{}"][source="{}"]'.format(
                            interface.circuit_id, interface.node_object.name
                        ),
                        "style": {
                    ...
                    new_entry_2 = {
                        "selector": 'edge[circuit_id="{}"][source="{}"]'.format(
                            interface.circuit_id, interface.remote_node_object.name
                        ),
                        "style": {
                            "width": str(self.demand_interface_width),
                            "line-style": "dashed",
                    ...
                    new_entry_3 = {
                        ...
                        "style": {"background-color": demand_color},
                    }

                    new_style.append(new_entry_3)

                    new_entry_4 = {
                        "selector": 'node[id="{}"]'.format(
            ...
            if selected_interface_info:
                if no_selected_interface_text not in selected_interface_info:
                    ...
                    new_entry_5 = {
                        ...
                        '[circuit_id="{}"]'.format(
                            selected_interface_info["source"],
        ...
        @app.callback(
            [
                Output("lsp-source-callback", "options"),
        ...
        def display_lsp_dropdowns(source, dest):
            ...
            if ctx_src_inputs is None and ctx_dest_inputs is None:
                ...
                dest_options = [
                    {"label": dest, "value": dest} for dest in lsp_destinations_list
                ]
                src_options = [
                    {"label": source, "value": source} for source in lsp_sources_list
                ]
                ...
                src_keys = [
                    src_key
                    for src_key in lsp_keys
                    if src_key.split("-")[1] == ctx_dest_inputs
                ]
                lsp_list = []
                ...
                dest_keys = [
                    dest_key
                    for dest_key in lsp_keys
                    if dest_key.split("-")[0] == ctx_src_inputs
                ]
                lsp_list = []
        ...
        @app.callback(
            [
                Output("demand-source-callback", "options"),
        ...
        def display_demand_dropdowns(source, dest):
            ...
            if ctx_src_inputs is None and ctx_dest_inputs is None:
                # No source or destination specified
                dest_options = [
                    {"label": dest, "value": dest} for dest in demand_destinations_list
                ]
                src_options = [
                    {"label": source, "value": source} for source in demand_sources_list
                ]
                ...
                msg = (
                    "Debug output: unaccounted for scenario in display_demand_dropdowns"
                )
                raise Exception(msg)
        ...
        @app.callback(
            Output("selected-lsp-output", "children"),
            [
        ...
        def display_selected_lsp(
            path_lsps, find_lsps, interface_lsps, clear_lsp_button
        ):
            """
            ...
            if ctx.triggered[0]["prop_id"] == "clear-lsp-button.n_clicks":
                ...
                selected_lsp = json.dumps({"label": no_selected_lsp_text, "value": ""})
            elif ctx.triggered[0]["value"] in [
                no_selected_demand_text,
                no_selected_interface_text,
            ]:
                selected_lsp = json.dumps({"label": no_selected_lsp_text, "value": ""})
            elif ctx.triggered[0]["value"] == json.dumps(
                [{"source": "", "dest": "", "name": ""}]
            ):
                raise PreventUpdate
            else:
                lsp_info = json.loads(ctx.triggered[0]["value"])
                lsp_object = model.get_rsvp_lsp(
                    lsp_info["source"], lsp_info["dest"], lsp_info["name"]
                )
        ...
        @app.callback(
            Output("selected-interface-output", "children"),
            [
        ...
        def display_selected_edge(
            map_edge, demand_interface, node_interface, lsp_interface, clear_int_button
        ):
            """
            ...
            if (
                ...
                end_target = [
                    item
                    for item in int_data["target"].split("-")[1:]
                    if item != int_data["source"]
                ][0]
                int_info = {
                    "source": int_data["source"],
                    "interface-name": int_data["interface-name"],
                    "dest": end_target,
                    "circuit_id": int_data["circuit_id"],
                    "utilization %": int_data["utilization"],
                    "cost": int_data["cost"],
                }
                ...
                if ctx.triggered[0]["value"] == no_selected_demand_text:
                    ...
                    if no_selected_demand_text not in int_data:
                        util = model.get_interface_object(
                            int_data["interface-name"], int_data["source"]
                        ).utilization
                        int_info = {
                            "source": int_data["source"],
                            "interface-name": int_data["interface-name"],
        ...
        @app.callback(
            Output("selected-demand-output", "children"),
            [
        ...
        def display_selected_demand_data(
            ...
            if ctx.triggered[0]["prop_id"] == "clear-dmd-button.n_clicks":
                ...
                demand_object = model.get_demand_object(
                    demand_info["source"], demand_info["dest"], demand_info["name"]
                )
        ...
        @app.callback(
            Output("interface-lsp-callback", "options"),
            [Input("selected-interface-output", "children")],
        )
        def lsps_on_interface(interface_info):
            ...
            if interface_info and no_selected_interface_text not in interface_info:
                ...
                interface = model.get_interface_object(
                    int_dict["interface-name"], int_dict["source"]
                )
                lsps = interface.lsps(model)
                if len(lsps) == 0:
                    return [
                        {
                            "label": "no lsps on interface",
                            "value": json.dumps(
                                [{"source": "", "dest": "", "name": ""}]
                            ),
                        }
        ...
        @app.callback(
            Output("interface-demand-callback", "options"),
            [Input("selected-interface-output", "children")],
        )
        ...
        # def that finds and displays demands on the selected interface
        @app.callback(
            Output("lsp-demand-callback", "options"),
            [Input("selected-lsp-output", "children")],
        )
        def demands_on_lsp(lsp_info):
            ...
            if (
                ...
                lsp = model.get_rsvp_lsp(
                    lsp_dict["source"], lsp_dict["dest"], lsp_dict["name"]
                )
                demands = lsp.demands_on_lsp(model)
                if len(demands) == 0:
                    return [
                        {
                            "label": "no demands on lsp",
                            "value": json.dumps(
                                [{"source": "", "dest": "", "name": ""}]
                            ),
                        }
        ...
        @app.callback(
            [
                Output("demand-path-interfaces", "options"),
        ...
        def demand_interfaces(demand):
            ...
            if demand:
                if (
                    ...
                    dmd = model.get_demand_object(
                        demand["source"], demand["dest"], demand["name"]
                    )
        ...
        @app.callback(
            Output("lsp-interface-callback", "options"),
            [Input("selected-lsp-output", "children")],
        )
        def lsp_interfaces(lsp):
            if lsp:
                if no_selected_lsp_text not in lsp and '"routed": false' not in lsp:
                    ...
                    return [
                        {"label": no_selected_lsp_text, "value": no_selected_lsp_text}
                    ]
            else:
