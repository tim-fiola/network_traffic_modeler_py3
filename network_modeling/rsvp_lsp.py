...
#            path_counter += 1
    ...
    # Calculate the effective LSP metric (shortest path)
    
    
    # Calculate the metric of the actual LSP path

# Treat the LSP like an interface with 
#   - reserved bandwidth (capacity)
#   - traffic (demands)

# Treat the LSP like a demand because it has a path
# If any interface in the path is overutilized, find a new path




# add a subroutine in model.py that converges the LSPs
#   - route demands on to LSPs first
#   - find route for LSP
#   - if LSP can only get demand partially to destination, calculate the
#       LSPs metric to that endpoint
#       - for this scenario, treat the LSP like an interface connected to the
#           node that's partially there

# for LSP implementation, will need to create a topology that includes the LSPs
# as interfaces, alongside the normal interfaces; run SPF calculations against that
# topology, then expand that topology back into the component interfaces that 
# make up the LSPs
#
#   shortest paths will have to be calculated with only LSPs . . . there's 
# a thought here i'm trying to form regarding running SPF with only the LSPs
# and then only the interfaces and comparing them . . . . 
